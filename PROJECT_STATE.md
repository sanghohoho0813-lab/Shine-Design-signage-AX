...
## v17 — 모바일 최우선 감사·수정 + 상시 가드 (최신)
- **자동 감사** 18라우트 × 360/390: 가로 넘침 · 36px 미만 터치 · 11.5px 미만 글자 · 이름 없는 버튼 · 하단 바 가림 → 전부 0 ✅ (QA 가드로 편입)
- 포트폴리오 상세 이전/다음 카드 454px 넘침(P0) 수정 ✅
- **터치 목표 44px** 전역 규칙(`button.tap`·radio·tab·switch) + 인라인 링크 `.tap-pad` ✅ · 배지 글자 최소 12px ✅
- **제작 발주** 폰에서 카드 목록(표는 sm부터) ✅ · **DemoBar** 폰에서 "AX →" 알약로 축소(폼 버튼 가림 해소) ✅
- 설정 Motion 스위치 접근 가능한 이름 · 44px ✅ · AX 본문 하단 safe-area 여백 ✅
- 폰 실제 조작 테스트: 발주 카드 상태 처리 · 문의 5단계 터치 완주 · 문의 현황 반영 ✅
- QA 298/298 · 회귀 0 ✅

## v16 — 테마 정리 · 메뉴 색 체계 · 폰 우선순위
...
## QA
- 헤드리스 인수 테스트 **298/298 PASS** (`scratchpad/qa.mjs`, v1~v17) — 모바일 가드 18×2 라우트 포함
